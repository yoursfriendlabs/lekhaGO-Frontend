...
import CategorySearchCreateField from "../../components/inventory/CategorySearchCreateField.jsx";
...
import {
  ...
  UNITS_SETTINGS_TAB,
} from "../../lib/business/settingsTabs.js";
...
import ImageCropperModal from "../../components/form/ImageCropperModal.jsx";
...
import ProductDetailDialog from "../../components/inventory/ProductDetailDialog.jsx";
...
const getInventoryItemActionId = (action, itemId) =>
  `inventory-item-${action}-${itemId}`;
...
export default function Inventory() {
  ...
  const itemTypeOptions =
    Array.isArray(inventoryProfile.itemTypes) &&
    inventoryProfile.itemTypes.length
  ...
  const inventorySubtitle =
    inventoryProfile.subtitle || t("inventory.itemsSubtitle");
  ...
  const handleDeleteProduct = async () => {
    ...
    try {
      ...
      setStatus({
        type: "success",
        message:
          t("inventory.messages.deleted") || "Product deleted successfully",
      });
      ...
      setStatus({
        ...
        message:
          err.message ||
          t("inventory.messages.deleteFailed") ||
  ...
  const items = useMemo(() => {
    return products.map((product) => ({
      ...
      unit: cleanUnitLabel(
        product.primaryUnit || getUnitText(product.unit) || "",
      ),
  ...
  const purityOptions = useMemo(
    () => getPurityOptionsForMetal(form.metalType),
    [form.metalType],
  );
  const primaryUnitChoices = useMemo(() => {
    ...
    const hasManagedSelection =
      form.unitId && unitOptionsById.has(String(form.unitId));
    const matchesExisting = current
      ? unitOptions.some(
          (unit) => getUnitText(unit).toLowerCase() === current.toLowerCase(),
        )
      : true;
  ...
  const hasPrimaryUnitSelected = Boolean(
    String(form.primaryUnit || "").trim() || form.unitId,
  );
    ...
    setForm((prev) => {
      ...
      return {
        ...
        purity:
          ...
          !nextPurityOptions.includes(prev.purity)
            ? ""
            : prev.purity,
      };
    });
  ...
  const openRestockDialog = (itemId) => {
    ...
    if (!isRestockableProduct(product)) {
      setStatus({
        type: "error",
        message: t("inventory.messages.serviceNoRestock"),
      });
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        setStatus({
          type: "success",
          message: t("inventory.messages.itemUpdated"),
        });
      } else {
        ...
        setStatus({
          type: "success",
          message: t("inventory.messages.itemCreated"),
        });
      }
      ...
      const categoryMissing =
        err?.status === 404 &&
        /categor/i.test(err?.message || err?.payload?.message || "");
      if (categoryMissing) {
        await loadCategories();
        setForm((previous) => ({ ...previous, categoryId: "" }));
        setStatus({
          type: "error",
          message: t("categories.messages.reselect"),
        });
      } else {
  ...
  const handleRestockSubmit = async (event) => {
    ...
    if (quantityValue <= 0) {
      setStatus({
        type: "error",
        message: t("inventory.messages.restockQuantityRequired"),
      });
      return;
    }
    ...
    try {
      ...
      setToast({
        ...
        message:
          ...
          t(
            isRemove
              ? "inventory.messages.itemStockReduced"
              : "inventory.messages.itemRestocked",
            {
  ...
  const handleCategoryCreated = useCallback((category) => {
    ...
    setCategoryOptions((previous) => {
      const next = previous.filter(
        (entry) => String(entry.id) !== String(category.id),
      );
      next.push(category);
      next.sort((a, b) =>
        String(a.name || "").localeCompare(String(b.name || "")),
  ...
  const currentRestockStock = restockProduct
    ? getCurrentStock(restockProduct)
    : 0;
  const restockQuantityValue = parseNumber(restockQuantity);
  const restockSignedDelta =
    restockAction === "remove" ? -restockQuantityValue : restockQuantityValue;
  const nextRestockStock = currentRestockStock + restockSignedDelta;
  const restockUnitSuffix = restockProduct?.primaryUnit
    ? ` ${restockProduct.primaryUnit}`
    : "";
    ...
    <div id="inventory-page" className="space-y-8">
      ...
      <div id="inventory-items-card" className="card">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <h3
            className="font-serif text-xl text-ink sm:text-2xl animate-fade-in"
            key={stockFilter}
          >
            {stockFilter === "low"
        ...
        <div className="mt-4 grid gap-2.5 grid-cols-1 sm:grid-cols-2 xl:grid-cols-[minmax(0,1.8fr)_repeat(3,minmax(0,1fr))] sm:gap-3">
          ...
          >
            <option value="name">
              {t("inventory.sortByName") || "Sort by Name"}
            </option>
            <option value="quantity">
              {t("inventory.sortByQuantity") || "Sort by Stock"}
            </option>
            <option value="salePrice">
              {t("inventory.sortBySalePrice") || "Sort by Sale Price"}
            </option>
            {canViewPurchasePrice ? (
              <option value="purchasePrice">
                {t("inventory.sortByPurchasePrice") || "Sort by Purchase Price"}
              </option>
            ) : null}
              ...
              >
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={`text-xs font-semibold rounded-full px-2 py-0.5 ${item.quantity <= 0 ? "bg-rose-100 text-rose-700" : item.quantity <= 5 ? "bg-amber-100 text-amber-700" : "bg-emerald-100 text-emerald-700"}`}
                    >
                ...
                {item.expiryDate && (
                  <div className="mt-2 text-xs text-secondary-500 px-0.5">
                    ...
                    <div
                      className={`mt-0.5 text-sm font-extrabold tracking-wide ${getExpiryDateColorClass(item.expiryDate)}`}
                    >
                      <DateDisplay date={item.expiryDate} />
                    </div>
                    <div className="text-xs font-bold mt-0.5">
                      {getExpiryRemainingDaysText(item.expiryDate, t)}
                    </div>
                  </div>
                )}
                <div className="mt-2.5 flex items-center justify-between gap-2 text-xs text-secondary-500 border-t border-secondary-100 pt-2.5 dark:border-slate-800">
                  <span className="min-w-0 truncate">
                    ...
                    <strong className="text-ink-light dark:text-secondary-300">
                  ...
                  {canViewPurchasePrice ? (
                    <span className="min-w-0 truncate text-right">
                      ...
                      <strong className="text-ink-light dark:text-secondary-300">
                        {t("currency.formatted", {
                          symbol: t("currency.symbol"),
                          amount: Number(item.purchasePrice || 0).toFixed(2),
                        })}
                      </strong>
                    </span>
                  ) : null}
          ...
          <table className="w-full text-sm text-secondary-700">
            ...
            <tbody>
              {productsLoading && products.length === 0 ? (
                <tr>
                  <td
                    colSpan={canViewPurchasePrice ? 10 : 9}
                    className="py-3 text-secondary-500"
                  >
                    {t("common.loading")}
                  </td>
                </tr>
                ...
                pagedItems.map((item) => (
                  <tr
                    key={item.id}
                    id={getInventoryItemRowId(item.id)}
                  ...
                  >
                      ...
                      <div className="flex items-center gap-3">
                        {item.imageUrl ? (
                          <img
                            src={item.imageUrl}
                            alt={item.name}
                            className="h-10 w-10 shrink-0 rounded-xl object-cover border border-secondary-200 dark:border-slate-800 cursor-zoom-in"
                        ...
                        <div>
                          ...
                          >
                            <p className="font-semibold text-ink hover:text-primary dark:text-white">
                              {item.name}
                            </p>
                          </button>
                    ...
                    <td className="py-3 text-left">
                      {item.expiryDate ? (
                        <div
                          className={`leading-snug ${getExpiryDateColorClass(item.expiryDate) || "text-secondary-500"}`}
                        >
                        ...
                        <ActionMenu
                          actions={[
                            ...
                            },
                            {
                              ...
                              icon: History,
                              onClick: () =>
                                openDetailDialog(item.id, "history"),
                            },
                            ...(canManageInventory && isRestockableProduct(item)
                              ? [
                                  {
      ...
      >
        ...
        >
          ...
          >
            <div className="space-y-3 sm:space-y-4">
              <div>
                ...
                <p
                  id="inventory-restock-item-name"
                  className="mt-1 break-words text-base font-semibold text-ink sm:text-lg"
                >
                  {restockProduct?.name || "-"}
                </p>
                <p
                  id="inventory-restock-item-unit"
                  className="text-sm text-secondary-500"
                >
                  {restockProduct?.primaryUnit || t("inventory.noUnit")}
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 sm:gap-4">
                <div>
                  ...
                  <div
                    id="inventory-restock-current-stock"
                    className="mt-1 rounded-xl border border-secondary-200/70 bg-mist px-3 py-2.5 text-sm font-medium text-ink-light dark:border-slate-800/70 dark:bg-slate-900/50 dark:text-slate-200 sm:rounded-2xl sm:px-4 sm:py-3"
                  >
                    {formatQuantity(currentRestockStock)}
                    {restockUnitSuffix}
                  </div>
                </div>
                <div>
                  <label className="label">
                    {isRestockRemove
                      ? t("inventory.restockQuantityRemove")
                      : t("inventory.restockQuantityAdd")}
                  </label>
                  <input
              ...
              {!isRestockRemove ? (
                <div className="space-y-3 rounded-xl border border-secondary-200/70 bg-mist/70 p-3 dark:border-slate-800/70 dark:bg-slate-900/40 sm:rounded-2xl">
                  ...
                  <div className="grid gap-3 sm:grid-cols-[1.1fr_1fr] sm:items-end sm:gap-4">
                    <div>
                      ...
                      <div className="mt-1">
                        <FlexibleDateInput
                          ...
                          onChange={(event) =>
                            setRestockExpiryDate(event.target.value || "")
                          }
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(event) =>
                          setRestockBatchNumber(event.target.value)
                        }
                        placeholder={
                  ...
                  <p className="text-xs text-secondary-500">
                    {t("inventory.restockExpiryHelp") ||
                      "If this stock has a new expiry/batch, add it here. Older stock keeps its own lot."}
                  </p>
                </div>
              ) : null}
              ...
              >
                ...
                <p
                  id="inventory-restock-new-stock"
                ...
                >
                  {formatQuantity(Math.max(nextRestockStock, 0))}
                  {restockUnitSuffix}
                </p>
                <p
          ...
          <div className="mobile-sticky-actions sticky bottom-0 z-10 -mx-4 -mb-4 flex flex-col-reverse gap-2 border-t border-secondary-200/70 bg-white/95 px-4 py-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] backdrop-blur dark:border-slate-800 dark:bg-slate-950/95 sm:static sm:mx-0 sm:mb-0 sm:flex-row sm:justify-end sm:border-0 sm:bg-transparent sm:p-0 sm:backdrop-blur-none">
            <button
              id="inventory-restock-close"
              className="btn-secondary w-full whitespace-normal text-center leading-tight sm:w-auto"
              type="button"
            ...
            <button
              id="inventory-restock-submit"
              className="btn-primary w-full whitespace-normal text-center leading-tight sm:w-auto"
              type="submit"
      ...
      <Dialog
        ...
        title={
          editingId
            ? `${t("common.edit")} ${t("inventory.itemName").toLowerCase()}`
                ...
                <div className="w-full md:w-1/3">
                  ...
                  {form.imageUrl ? (
                    <div className="relative group rounded-2xl overflow-hidden border border-secondary-200 dark:border-slate-800 aspect-square w-full bg-mist flex items-center justify-center">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <button
                          ...
                          onClick={() =>
                            document
                              .getElementById("inventory-image-input")
                              .click()
                          }
                        >
                          Change
                    ...
                    <div
                      ...
                      onClick={() =>
                        document.getElementById("inventory-image-input").click()
                      }
                    >
                      {imageUploading ? (
                        ...
                        <div className="flex flex-col items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-ink-light dark:text-secondary-300">
                            Upload image
                          </span>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div>
                      <label className="label">
                        ...
                        <span className="ml-1 text-[10px] text-secondary-400 font-normal">
                          (barcode)
                        </span>
                      </label>
                      <input
                ...
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-secondary-700">
                      {editingId
                        ? t("inventory.stockLotsEditHint") ||
                          "Product details stay the same. Stock is tracked in lots below."
                        : t("inventory.stockLotsCreateHint") ||
                          "Optional opening lot. Later restock/purchase can add more lots."}
                    </p>
                  </div>
                  {editingId ? (
                  ...
                  <div className="mt-4 space-y-2">
                    ...
                    {editBatches.length > 0 ? (
                      editBatches.map((batch, batchIndex) => {
                        const batchExpiry =
                          toDateInputValue(batch.expiryDate) || "";
                        return (
                          <div
                            ...
                            <div className="min-w-0">
                              <div className="font-medium text-ink-light">
                                {batch.batchNumber
                                  ...
                                  : t("inventory.noBatchNumber") ||
                                    "No batch no."}
                              </div>
                              <div
                                className={
                                  batchExpiry
                                    ? getExpiryDateColorClass(batchExpiry)
                                    : "text-secondary-500"
                                }
                              >
                                {batchExpiry
      ...
      <ConfirmDialog
        ...
        confirming={
          Boolean(deleteProduct) && deletingProductId === deleteProduct.id
        }
      />
    </div>
  );
