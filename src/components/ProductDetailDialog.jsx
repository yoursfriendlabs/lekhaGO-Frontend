...
function DetailRow({ label, children }) {
  return (
    <div className="flex items-start justify-between gap-3 border-b border-secondary-100 py-2.5 last:border-0 dark:border-slate-800">
      <span className="shrink-0 text-xs font-medium uppercase tracking-wide text-secondary-400">{label}</span>
      <div className="min-w-0 text-right text-sm font-medium text-ink dark:text-slate-200">{children}</div>
    </div>
  );
}
                ...
                return (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className={`text-xs ${expiry ? getExpiryDateColorClass(expiry) : 'text-secondary-500'}`}>
                        {expiry ? (
                          <>
                  ...
                  {historyItems.map((log) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {log.note ? (
                          <p className="mt-2 break-words text-xs text-secondary-500">{log.note}</p>
                        ) : null}
                      </div>
                    );
                  })}
                  ...
                  <table className="w-full text-sm text-secondary-700">
                    ...
                    <tbody>
                      {historyItems.map((log) => {
                        ...
                        const isAddition = qty > 0;
                        return (
                          <tr key={log.id} className="border-b border-secondary-100/75 dark:border-slate-800">
                            ...
                            <td className="max-w-xs truncate py-3 pl-4 text-xs text-secondary-500" title={log.note}>
                              {log.note || '—'}
                            </td>
                          </tr>
                        );
