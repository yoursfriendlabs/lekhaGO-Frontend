...
export default function BanksSettingsPanel() {
  const { t } = useI18n();
  const {
    banks,
    total,
  ...
  const handleDelete = async (bank) => {
    if (!window.confirm(t('banks.deleteConfirm', { name: bank.name || t('banks.unnamed') }))) {
      return;
    }
  ...
  return (
    <section className="space-y-4">
      ...
      <Dialog
        isOpen={dialogOpen}
        onClose={closeDialog}
        title={editingId ? t('banks.editBank') : t('banks.addBank')}
        size="md"
      >
        <form className="space-y-4" onSubmit={submitForm}>
          ...
          <div className="grid gap-3 md:grid-cols-2">
            ...
            <div>
              ...
              <input
                className="input mt-1"
                value={editingId ? form.currentBalance : form.openingBalance}
                readOnly
                disabled
              />
            </div>
