...
function getBusinessName(business, user, settings) {
  return business?.name || business?.businessName || settings?.companyName || user?.businessName || '';
}

export default function ProfileSettingsPanel({ isOwner = false }) {
  const { t } = useI18n();
  const { user, business, syncSession } = useAuth();
  const { settings, syncBusinessName } = useBusinessSettings();
  ...
  useEffect(() => {
    setForm({
      name: user?.name || '',
      phone: user?.phone || '',
      businessName: getBusinessName(business, user, settings),
    });
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const payload = {
        ...
        businessName: isOwner ? form.businessName.trim() : getBusinessName(business, user, settings),
      };
      ...
      if (isOwner && payload.businessName) {
        syncBusinessName(nextSnapshot?.businessId, nextSnapshot?.business || { name: payload.businessName, businessName: payload.businessName });
      }
