...
import { EMPTY_STAFF_SUMMARY, normalizeStaffMeta } from "../lib/staff";
...
function StatusBadge({ active, t }) {
  return (
    ...
    >
      {active
        ? t("staffManagement.status.active")
        : t("staffManagement.status.inactive")}
    </span>
  );
}
...
function LoginBadge({ hasLogin, t }) {
  return (
    ...
    >
      {hasLogin
        ? t("staffManagement.loginAccess")
        : t("staffManagement.nonLoginStaff")}
    </span>
  );
}
...
function PermissionSelector({
  ...
  const visibleLevels = levels.filter(
    (level) => !hideLevels.includes(level.key),
  );
  const columnClass = visibleLevels.length <= 2 ? "grid-cols-2" : "grid-cols-3";

  return (
    <div
      className={`grid ${columnClass} overflow-hidden rounded-2xl border border-secondary-200/70 bg-mist/80 dark:border-slate-800/70 dark:bg-slate-900/70`}
    >
...
function StaffFormDialog({
  ...
  const includeCafeModules =
    businessProfile?.modules?.orders === true ||
  ...
  const validateDetailsStep = () => {
    ...
    if (
      nameField &&
      typeof nameField.reportValidity === "function" &&
              ...
              <div className="mx-auto w-full max-w-[920px] space-y-4">
                <div className="flex gap-2 border-b border-secondary-200 pb-3 dark:border-slate-800">
                  ...
                  <button
                    type="button"
                    onClick={() => {
                      if (readOnly || validateDetailsStep())
                        setActiveTab("permissions");
                    }}
                    className={`rounded-2xl px-4 py-2 text-sm font-semibold transition ${
                ...
                <div className={`space-y-4 ${isDetailsStep ? "" : "hidden"}`}>
                  <div className="rounded-[28px] border border-secondary-200/80 bg-white/95 p-5 shadow-sm shadow-slate-200/20 dark:border-slate-800/70 dark:bg-slate-950/40 md:p-6">
                    <div className="flex flex-col gap-3 border-b border-secondary-200/50 pb-4 lg:flex-row lg:items-start lg:justify-between dark:border-slate-800/50">
                      <div>
                        <h3 className="font-serif text-lg text-ink">
                          {t("staffManagement.detailsTitle")}
                        </h3>
                        <p className="mt-1 text-xs text-secondary-500">
                          {t("staffManagement.detailsSubtitle")}
                        </p>
                      </div>
                      {readOnly ? (
                    ...
                    <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                      ...
                      <div>
                        ...
                        >
                          <option value="staff">
                            {t("staffManagement.roles.staff")}
                          </option>
                          {form.role === "owner" ? (
                            <option value="owner">
                              {t("staffManagement.roles.owner")}
                            </option>
                          ) : null}
                        </select>
                        <p className="mt-1 text-xs text-secondary-500">
                          {t("staffManagement.roleHelper")}
                        </p>
                      </div>
                      <div>
                        <label className="label" htmlFor="staff-job-title">
                          {t("staffManagement.jobTitle")}
                        </label>
                        <input
                          id="staff-job-title"
                          className="input mt-1"
                          value={form.jobTitle}
                          onChange={(event) =>
                            onFieldChange("jobTitle", event.target.value)
                          }
                          disabled={readOnly}
                      ...
                      <div>
                        <label className="label" htmlFor="staff-joined-date">
                          {t("staffManagement.joinedDate")}
                        </label>
                        <FlexibleDateInput
                          ...
                          onChange={(event) =>
                            onFieldChange("joinedDate", event.target.value)
                          }
                          disabled={readOnly}
                        />
                      </div>
                      <div>
                        <label className="label" htmlFor="staff-shift">
                          {t("staffManagement.shift")}
                        </label>
                        <input
                          id="staff-shift"
                          className="input mt-1"
                          value={form.shift}
                          onChange={(event) =>
                            onFieldChange("shift", event.target.value)
                          }
                          disabled={readOnly}
                      ...
                      <div>
                        <label className="label" htmlFor="staff-shift-started">
                          {t("staffManagement.shiftStarted")}
                        </label>
                        <input
                          ...
                          onChange={(event) =>
                            onFieldChange("shiftStarted", event.target.value)
                          }
                          disabled={readOnly}
                        />
                      </div>
                      <div>
                        <label className="label" htmlFor="staff-shift-ended">
                          {t("staffManagement.shiftEnded")}
                        </label>
                        <input
                          ...
                          onChange={(event) =>
                            onFieldChange("shiftEnded", event.target.value)
                          }
                          disabled={readOnly}
                        />
                      </div>
                  ...
                  <div className="rounded-[28px] border border-secondary-200/80 bg-white/95 p-5 shadow-sm shadow-slate-200/20 dark:border-slate-800/70 dark:bg-slate-950/40 md:p-6">
                    <div className="border-b border-secondary-200/50 pb-4 dark:border-slate-800/50">
                      ...
                      <p className="mt-1 text-xs text-secondary-500">
                        Manage payment and salary tracking parameters for this
                        staff member.
                      </p>
                    </div>
                    ...
                    <div className="mt-5 grid gap-4 md:grid-cols-2">
                      <div className={isCreate ? "md:col-span-2" : ""}>
                        <label className="label" htmlFor="staff-salary">
                          {t("staffManagement.salary")}
                        </label>
                        <input
                          id="staff-salary"
                          ...
                          disabled={readOnly}
                          placeholder={t(
                            "staffManagement.compensationPlaceholder",
                          )}
                        />
                      </div>
                      {!isCreate ? (
                ...
                <div className={`space-y-4 ${isDetailsStep ? "hidden" : ""}`}>
                  <div className="rounded-[28px] border border-secondary-200/80 bg-white/95 p-5 shadow-sm shadow-slate-200/20 dark:border-slate-800/70 dark:bg-slate-950/40 md:p-6">
                    <div className="border-b border-secondary-200/50 pb-4 dark:border-slate-800/50">
                      ...
                      <p className="mt-1 text-xs text-secondary-500">
                        Control if this staff member can log in to perform
                        transactions or view workspace features.
                      </p>
                    </div>
                      ...
                      <div className="flex items-center justify-between rounded-2xl border border-secondary-200/70 bg-white/80 p-4 dark:border-slate-800/70 dark:bg-slate-950/50">
                        <div>
                          <p className="text-sm font-semibold text-ink">
                            {t("staffManagement.hasLogin")}
                          </p>
                          <p className="mt-1 text-xs text-secondary-500">
                            If enabled, credentials are required to sign in.
                          </p>
                        </div>
                        <button
                          type="button"
                          disabled={readOnly}
                          onClick={() =>
                            onFieldChange("hasLogin", !form.hasLogin)
                          }
                          className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 ${
                            form.hasLogin
                              ? "bg-primary-600"
                              : "bg-secondary-200 dark:bg-slate-800"
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          {isCreate || wasNonLogin ? (
                            ...
                            <div className="rounded-2xl border border-secondary-200/70 bg-white/80 p-4 dark:border-slate-800/70 dark:bg-slate-950/50">
                              ...
                              <p className="mt-1 text-xs text-secondary-500">
                                {t("staffManagement.emailImmutable")}
                              </p>
                            </div>
                          )}
                          <div>
                            ...
                            <input
                              ...
                              placeholder={
                                isCreate
                                  ? t("staffManagement.passwordCreateHint")
                                  : t("staffManagement.passwordEditHint")
                              }
                  ...
                  <div className="rounded-[28px] border border-secondary-200/80 bg-white/95 p-5 shadow-sm shadow-slate-200/20 dark:border-slate-800/70 dark:bg-slate-950/40 md:p-6">
                    <div>
                      <h3 className="font-serif text-xl text-ink">
                        {t("staffManagement.permissionsTitle")}
                      </h3>
                    ...
                    <div className="mt-4 rounded-2xl border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-900 dark:border-sky-800/50 dark:bg-sky-950/30 dark:text-sky-100">
                      <p className="font-semibold">
                        {t("staffManagement.permissionDependencyTitle")}
                      </p>
                      <p className="mt-1 text-xs opacity-80">
                        {t("staffManagement.permissionDependencyHint")}
                      </p>
                    </div>
                        ...
                        <div key={group.id} className="space-y-3">
                          <div>
                            <h4 className="text-sm font-semibold text-ink">
                              {t(
                                `staffManagement.permissionGroups.${group.id}`,
                              ) || group.id}
                            </h4>
                            {(() => {
                          ...
                          <div className="grid gap-4 xl:grid-cols-2">
                            {group.features.map((feature) => {
                              const permissionKey =
                                getPermissionKeyForFeature(feature.key) ||
                                feature.key;
                              ...
                              const featureHint =
                                isInventory && inventoryRequired
                                  ? t(
                                      "staffManagement.permissionInventoryRequiredHint",
                                    )
                                  : isParties && requiredPartiesLevel !== "none"
                                    ? t(
                                        "staffManagement.permissionPartiesRequiredHint",
                                      )
                                    : translatedHint !== hintKey
                                      ? translatedHint
                                      : feature.description ||
                                        t(
                                          "staffManagement.permissionDescriptionFallback",
                                        );
                              const hideLevels = [
                                ...(isInventory && inventoryRequired
                                  ? ["none"]
                              ...
                              return (
                                <div
                                  key={permissionKey}
                                  className="rounded-2xl border border-secondary-200/70 bg-white/80 p-4 dark:border-slate-800/70 dark:bg-slate-950/50"
                                >
...
export default function StaffManagement({ businessId }) {
  ...
  const staffOverview = useMemo(() => {
    ...
    const loginCount = members.filter(
      (member) => member.hasLogin !== false,
    ).length;
  ...
  const reloadStaff = async () => {
    ...
    try {
      ...
      setMembers([]);
      setLoadError(
        normalizeErrorMessage(error, t("staffManagement.permissionError")),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handlePermissionChange = (featureKey, level) => {
    setForm((current) => ({
      ...current,
      permissions: applyPermissionChange(
        current.permissions,
        featureKey,
        level,
      ),
  ...
  const handleSubmit = async (event) => {
    ...
    const payload = {
      ...
      totalReceived:
        form.totalReceived === "" ? null : Number(form.totalReceived),
    ...
    if (dialogMode === "create") {
      ...
      const originalMember = members.find(
        (m) => m.membershipId === form.membershipId,
      );
      const wasNonLogin = originalMember
        ? originalMember.hasLogin === false
        : false;

      if (form.hasLogin) {
        if (
          wasNonLogin ||
          form.email.trim() !== (originalMember?.user?.email || "")
        ) {
          payload.email = form.email.trim();
        }
    ...
    try {
      if (dialogMode === "edit" && form.membershipId) {
        ...
        setNotice({
          type: "success",
          message: t("staffManagement.messages.updated"),
        });
      } else {
        await api.createStaff(payload);
        setNotice({
          type: "success",
          message: t("staffManagement.messages.created"),
        });
      }
      ...
      setNotice({
        ...
        message: normalizeErrorMessage(
          error,
          t("staffManagement.permissionError"),
        ),
      });
    } finally {
      setSaving(false);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setNotice({
        type: "success",
        message: t("staffManagement.messages.deleted"),
      });
      await reloadStaff();
    } catch (error) {
      setNotice({
  ...
  return (
    <div className="space-y-6">
      ...
      {notice.message ? (
        <Notice title={notice.message} tone={notice.type || "info"} />
      ) : null}
      ...
      {!canManageStaff ? (
        <Notice title={t("staffManagement.viewOnlyNotice")} tone="info" />
      ) : null}
        ...
        <div className="px-4 py-4 md:px-6 md:py-6">
          {loading ? (
            ...
            <div className="rounded-3xl border border-dashed border-slate-300/80 bg-mist/80 p-10 text-center dark:border-slate-700 dark:bg-slate-900/50">
              <h3 className="font-serif text-xl text-ink">
                {t("staffManagement.emptyTitle")}
              </h3>
              <p className="mt-2 text-sm text-secondary-500">
                {t("staffManagement.emptyDescription")}
              </p>
            </div>
          ) : (
            <>
                  ...
                  return (
                    ...
                    >
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="mt-1 flex flex-wrap items-center gap-1.5">
                              <span className="text-xs text-secondary-500">
                                {t(
                                  `staffManagement.roles.${member.role === "owner" ? "owner" : "staff"}`,
                                )}
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <StatusBadge
                          active={member.user?.isActive !== false}
                          t={t}
                        />
                        ...
                        {member.shiftStarted || member.shiftEnded ? (
                          <span className="text-xs text-secondary-500">
                            {t("staffManagement.shift")}:{" "}
                            {toTimeInputValue(member.shiftStarted) || "-"} -{" "}
                            {toTimeInputValue(member.shiftEnded) || "-"}
                          </span>
                        ) : null}
                      </div>
                      ...
                      <div className="mt-4 flex items-center justify-between border-t border-secondary-200/70 pt-3 dark:border-slate-800/70">
                        <span className="text-xs text-secondary-500">
                          {t("staffManagement.joined", "Joined")}:{" "}
                          {formatDate(member.joinedAt || member.joinedDate)}
                        </span>
                        <ActionMenu
                          actions={menuActions}
                  ...
                  <thead className="text-left text-xs uppercase tracking-[0.18em] text-ink">
                    <tr>
                      <th className="py-2 pr-4">
                        {t("staffManagement.employee", "Employee")}
                      </th>
                      <th className="py-2 pr-4">
                        {t("staffManagement.contact", "Contact")}
                      </th>
                      <th className="py-2 pr-4">
                        {t("staffManagement.roleJoined", "Role & Joined")}
                      </th>
                          ...
                          <td className="py-3 pr-4">
                            <div className="space-y-1">
                              {member.hasLogin ? (
                                ...
                                <p className="italic text-secondary-400">
                                  {t("staffManagement.noLoginPlaceholder")}
                                </p>
                              )}
                          ...
                          <td className="py-3 pr-4">
                            <div className="space-y-1">
                              <p className="font-medium capitalize text-ink dark:text-slate-200">
                                {t(
                                  `staffManagement.roles.${member.role || "staff"}`,
                                )}
                              </p>
                              <p className="text-xs text-secondary-500">
                                {t("staffManagement.joined", "Joined")}:{" "}
                                {formatDate(
                                  member.joinedAt || member.joinedDate,
                                )}
                              </p>
                              {member.shiftStarted || member.shiftEnded ? (
                                <p className="text-xs text-secondary-500">
      ...
      <StaffFormDialog
        ...
        wasNonLogin={
          form.membershipId
            ? members.find((m) => m.membershipId === form.membershipId)
                ?.hasLogin === false
            : false
        }
        onClose={closeDialog}
      ...
      <ConfirmDialog
        ...
        description={
          deleteMember
            ? t("staffManagement.deleteDescription", {
                name:
                  deleteMember.user?.name || deleteMember.user?.email || "-",
              })
...
function SalaryAdvanceDialog({ isOpen, member, t, onClose }) {
  ...
  const stats = useMemo(() => {
    ...
    const netRemaining =
      monthlySalary - totalAdvanceThisMonth - totalPaidThisMonth;
  ...
  return (
    <Dialog
      isOpen={isOpen}
      onClose={onClose}
      title={t("staffManagement.salaryRecords.title", {
      ...
      <div className="space-y-6">
        ...
        <div className={STATS_GRID_CLASS}>
          <StatsCard
            ...
            value={t("currency.formatted", {
              symbol: t("currency.symbol"),
              amount: stats.monthlySalary.toFixed(2),
            })}
            icon={DollarSign}
            tone="default"
          />
          <StatsCard
            ...
            value={t("currency.formatted", {
              symbol: t("currency.symbol"),
              amount: stats.totalAdvanceThisMonth.toFixed(2),
            })}
            icon={DollarSign}
            tone="warning"
          />
          <StatsCard
            ...
            value={t("currency.formatted", {
              symbol: t("currency.symbol"),
              amount: stats.totalPaidThisMonth.toFixed(2),
            })}
            icon={Check}
            tone="success"
          />
          <StatsCard
            ...
            value={t("currency.formatted", {
              symbol: t("currency.symbol"),
              amount: stats.netRemaining.toFixed(2),
            })}
            icon={ShieldCheck}
        ...
        {!adding ? (
          ...
          >
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
              <div>
                <label className="label" htmlFor="record-amount">
                  {t("staffManagement.salaryRecords.amount")}
                </label>
                <input
                  id="record-amount"
                  type="number"
              ...
              <div>
                <label className="label" htmlFor="record-type">
                  {t("staffManagement.salaryRecords.type")}
                </label>
                <select
                  id="record-type"
                  className="input mt-1"
              ...
              <div>
                <label className="label" htmlFor="record-date">
                  {t("staffManagement.salaryRecords.date")}
                </label>
                <FlexibleDateInput
                  id="record-date"
                  className="input mt-1"
            ...
            <div>
              <label className="label" htmlFor="record-note">
                {t("staffManagement.salaryRecords.note")}
              </label>
              <input
                id="record-note"
                type="text"
              ...
              <table className="w-full text-sm text-secondary-700">
                ...
                <tbody className="divide-y divide-slate-200/60 dark:divide-slate-800/60">
                  {records.map((r) => (
                    <tr
                      key={r.id}
                      className="hover:bg-mist/40 dark:hover:bg-slate-900/10"
                    >
                      ...
                      <td className="max-w-[200px] truncate p-3" title={r.note}>
                        {r.note || "-"}
                      </td>
