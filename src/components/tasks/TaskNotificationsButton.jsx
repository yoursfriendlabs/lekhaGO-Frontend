...
import { formatMaybeDateTime } from '../../lib/dates/datetime';

function CounterPill({ label, value }) {
  return (
    <div className="rounded-2xl border border-secondary-200/70 bg-mist/80 px-3 py-2 dark:border-slate-800/70 dark:bg-slate-900/70">
      <p className="text-[11px] uppercase tracking-[0.14em] text-secondary-400">{label}</p>
...
export default function TaskNotificationsButton({ t }) {
  ...
  return (
    <div ref={rootRef} className="relative">
      <button
        type="button"
        className="relative inline-flex min-h-[42px] min-w-[42px] items-center justify-center rounded-2xl border border-secondary-200 bg-white px-3 py-2 text-ink-light transition-transform active:scale-95"
      ...
      {open ? (
        <div className="absolute right-0 top-[calc(100%+0.75rem)] z-30 w-[min(92vw,25rem)] overflow-hidden rounded-3xl border border-secondary-200/80 bg-white shadow-2xl shadow-slate-900/10 dark:border-slate-800/70 dark:bg-slate-950">
          <div className="flex items-start justify-between gap-3 border-b border-secondary-200/70 px-4 py-4 dark:border-slate-800/70">
            <div>
              <p className="text-xs uppercase tracking-[0.18em] text-secondary-400">{t('tasks.notifications.eyebrow')}</p>
              <h3 className="mt-1 font-serif text-lg text-ink">{t('tasks.notifications.title')}</h3>
          ...
          <div className="border-t border-secondary-200/70 px-4 py-4 dark:border-slate-800/70">
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-semibold text-ink">{t('tasks.notifications.recentActivity')}</p>
              ...
              <div className="mt-3 space-y-3">
                {summary.recentActivities.slice(0, 5).map((activity) => (
                  <Link
                    ...
                    className="block rounded-2xl border border-secondary-200/70 bg-mist/80 px-4 py-3 transition hover:border-primary-200 hover:bg-primary-50/50 dark:border-slate-800/70 dark:bg-slate-900/70 dark:hover:border-primary-500/30 dark:hover:bg-primary-500/10"
                  >
                    ...
                    {activity.content ? (
                      <p className="mt-2 line-clamp-2 text-sm text-secondary-700">{activity.content}</p>
                    ) : null}
                  </Link>
                ))}
              </div>
