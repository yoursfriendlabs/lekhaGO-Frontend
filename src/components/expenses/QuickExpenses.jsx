...
import QuickPartySelector from "../parties/QuickPartySelector.jsx";
...
export default function QuickExpenseForm({
  ...
  const canManageQuickExpenses =
    canManageFeature("quickExpenses") || canManagePurchases;
  const canViewParties = canViewFeature("parties");
  const { invalidate: invalidatePurchases, fetch: fetchPurchases } =
    usePurchaseStore();
  const {
    categories,
  ...
  const resolveCatMeta = (line) => {
    return (
      categories.find((category) => category.id === line.categoryId) || null
    );
  };

  const handleAddCategory = () => {
    const id = addCustomCategory(newCategoryName);
    if (id && quickLines[0]) {
      updateLine(quickLines[0]._key, "categoryId", id);
      updateLine(quickLines[0]._key, "customCategory", newCategoryName.trim());
    }
  ...
  const handleQuickExpenseSubmit = async (event) => {
    ...
    if (!businessId) {
      setQuickStatus({
        type: "error",
        message: t("errors.businessIdRequired"),
      });
      return;
    }
    ...
    if (!quickHeader.expenseDate) {
      setQuickStatus({
        type: "error",
        message: t("errors.purchaseDateRequired"),
      });
      return;
    }

    const validLines = quickLines.filter(
      (line) => line.categoryId && Number(line.amount || 0) > 0,
    );
    ...
    if (
      ...
      setQuickStatus({
        type: "error",
        message: t("quickExpense.categoryRequired"),
      });
      return;
    }
    ...
    if (
      validLines.some(
        (line) =>
          line.categoryId === CUSTOM_EXPENSE_CATEGORY &&
          !line.customCategory?.trim(),
      )
    ) {
      setQuickStatus({
        type: "error",
        message: t("quickExpense.customCategoryRequired"),
      });
      return;
    }
    ...
    try {
      ...
      setQuickStatus({
        type: "error",
        message: err.message || t("quickExpense.saveFailed"),
      });
    } finally {
      ...
      <form className="space-y-6" onSubmit={handleQuickExpenseSubmit}>
          ...
          {canViewParties ? (
            <div className="rounded-[26px] border border-secondary-200/80 bg-mist/70 p-4 dark:border-slate-800/70 dark:bg-slate-900/40">
              <label className="label">
                {t("quickExpense.linkPartyOptional")}
              </label>
              <button
                type="button"
            ...
            return (
              ...
              >
                <div className="mb-4 flex items-center gap-3">
                  <span
                    className={`flex h-9 w-9 items-center justify-center rounded-2xl text-sm ${catMeta ? catMeta.iconWrap : "bg-secondary-100 text-secondary-400"}`}
                  >
                    <CatIcon size={13} />
                  </span>
                  <div>
                ...
                <div className="mb-5 grid grid-cols-2 gap-3 sm:grid-cols-3 xl:grid-cols-4">
                  {categories.map((category) => {
                    ...
                    const isSavedCustom =
                      category.id.startsWith("saved-custom-");
                    const isRemovable =
                      ["food", "transport", "utilities"].includes(
                        category.id,
                      ) || isSavedCustom;
                    ...
                    return (
                      <div key={category.id} className="relative">
                        <button
                          ...
                          onClick={() => {
                            if (isSavedCustom) {
                              updateLine(line._key, "categoryId", category.id);
                              updateLine(
                                line._key,
                                "customCategory",
                                category.label,
                          ...
                          className={`flex min-h-[96px] w-full flex-col items-center justify-center gap-2 rounded-2xl border px-3 py-3 transition ${isActive ? `border-2 ${category.activeColor}` : "border-secondary-200 bg-mist hover:border-secondary-300 dark:border-slate-800 dark:bg-slate-900/50 dark:hover:border-slate-700"}`}
                        >
                          <span
                            className={`flex h-10 w-10 items-center justify-center rounded-xl ${isActive ? category.iconWrap : "bg-secondary-100 text-secondary-500 dark:bg-slate-800 dark:text-secondary-400"}`}
                          >
                            <Icon size={16} />
                          </span>
                        ...
                        {isRemovable ? (
                          <button
                            ...
                            className="absolute right-1 top-1 flex h-6 w-6 items-center justify-center rounded-full bg-secondary-200/90 text-secondary-600 transition hover:bg-red-500 hover:text-white dark:bg-slate-700 dark:text-secondary-300 dark:hover:bg-red-600 dark:hover:text-white"
                          >
                            <X size={12} />
                          </button>
                        ) : null}
                  ...
                  <button
                    ...
                    className="flex min-h-[96px] flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-secondary-300 bg-transparent px-3 py-3 text-secondary-400 transition hover:border-primary hover:bg-primary-50/40 hover:text-primary dark:border-slate-700 dark:text-secondary-400 dark:hover:border-primary dark:hover:bg-primary-900/10 dark:hover:text-primary"
                  >
                    <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-secondary-100 text-secondary-500 dark:bg-slate-800 dark:text-secondary-400">
                      <Plus size={18} />
                    </span>
                ...
                {addingCategory ? (
                  <div className="mb-5 rounded-[24px] border border-primary-200 bg-primary-50/50 p-4 dark:border-primary-900/40 dark:bg-primary-900/10">
                    <p className="mb-3 text-sm font-semibold text-ink">
                      {t("Add a custom expense category") ||
                        "Add a custom expense category"}
                    </p>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                      <input
                        autoFocus
                        className="input flex-1 rounded-[16px]"
                        type="text"
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        <button
                          type="button"
                          className="btn-primary rounded-[16px]"
                        ...
                        <button
                          type="button"
                          className="btn-ghost rounded-[16px]"
                          onClick={() => {
                            setAddingCategory(false);
                            setNewCategoryName("");
                          }}
                ...
                <div className="grid gap-4 lg:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
                  <div className="rounded-[24px] border border-secondary-200/70 bg-mist/70 p-4 dark:border-slate-800/70 dark:bg-slate-900/40">
                    ...
                    <input
                      ...
                      value={line.amount}
                      onChange={(event) =>
                        updateLine(line._key, "amount", event.target.value)
                      }
                    />
                  </div>
                  ...
                  <div className="rounded-[24px] border border-secondary-200/70 bg-mist/70 p-4 dark:border-slate-800/70 dark:bg-slate-900/40">
                    ...
                    <input
                      ...
                      value={line.notes}
                      onChange={(event) =>
                        updateLine(line._key, "notes", event.target.value)
                      }
                    />
                  </div>
                </div>
                ...
                {lineAmount > 0 && catMeta && line.categoryId ? (
                  <div className="mt-4 flex items-center justify-between rounded-2xl bg-mist px-4 py-3 dark:bg-slate-900/60">
                    <span
                      className={`inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-semibold ${catMeta.badge}`}
                    >
                      <CatIcon size={10} />
          ...
          <div className="rounded-[28px] border border-emerald-200 bg-emerald-50 p-5 dark:border-emerald-900/40 dark:bg-emerald-950/30">
            ...
            <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <p className="text-3xl font-bold text-ink">
                {money(quickGrandTotal)}
              </p>
              <p className="text-sm text-emerald-800/80 dark:text-emerald-200/80">
                {t("quickEntry.liveTotal")}
              </p>
            </div>
          </div>
        ) : null}
        ...
        <div className="rounded-[28px] border border-secondary-200/80 bg-mist p-5 dark:border-slate-800/70 dark:bg-slate-900/50">
          ...
          <PaymentMethodFields
            ...
            onChange={(patch) =>
              setQuickHeader((prev) => ({ ...prev, ...patch }))
            }
          />
        </div>

        {quickStatus.message ? (
          <Notice title={quickStatus.message} tone={quickStatus.type} />
        ) : null}
